        ...
        <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row justify-between w-full h-full px-6 md:px-12 relative z-10">
          <div className="mb-20 md:mb-0">
            <div className="flex items-center gap-2 group cursor-pointer relative text-sand">
              <span className="font-caveat text-4xl leading-none absolute -top-4 -left-6 transform -rotate-12 z-10 text-royal-red">RB</span>
              <div className="flex flex-col">
                <span className="font-playfair text-3xl tracking-[0.1em] leading-[0.85]">ROYAL</span>
                <span className="font-playfair text-[10px] tracking-[0.2em] leading-none mt-1">BEVERAGE</span>
              </div>
            </div>
          </div>
          ...
          >
            {navLinks.map((link) => (
              <motion.a 
                ...
                key={link.key} 
                href={link.key === 'HOME' ? '/' : link.key === 'PRODUCTION' ? '/production' : link.key === 'TRACEABILITY' ? '/traceability' : link.key === 'ABOUT' ? '/about' : `#${link.key.toLowerCase().replace(/ /g, '-')}`} 
      ...
      ></div>
        ...
        >          
          <div className="w-full overflow-hidden mt-auto pt-20 pb-4">
            <h1 
              className="font-playfair font-bold text-[#1e1e1e] text-[10vw] sm:text-[10vw] md:text-[12vw] leading-[0.75] select-none w-full align-center text-center uppercase"                              
            >
              K I L O L U A
            </h1>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-0 mt-8 mb-12">
            <div className="flex flex-wrap gap-6 md:gap-10 font-lora text-xs font-bold opacity-80">              
              <a href="#" onClick={(e) => { e.preventDefault(); window.dispatchEvent(new Event('openContact')); }} className="hover:text-royal-red transition-colors uppercase">{t('footer.contact')}</a>
              <a href="#privacy" className="hover:text-royal-red transition-colors uppercase">{t('footer.privacy')}</a>
              <a href="#terms" className="hover:text-royal-red transition-colors uppercase">{t('footer.terms')}</a>
            </div>
